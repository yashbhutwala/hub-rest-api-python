...
class DockerWrapper:
    ...
    def pull_container_image(self, image_name):
        args = [self.docker_path, 'pull', image_name]
        return subprocess.run(args)

    def save_container_image(self, image_name):
        args = [self.docker_path, 'save', '-o', self.imagefile, image_name]
        return subprocess.run(args)

    def unravel_container(self):
        args = ['tar', 'xvf', self.imagefile, '-C', self.imagedir]
        return subprocess.run(args)
...
class Detector:
    ...
    def detect_run(self, options=['--help']):
        cmd = ['bash', self.filename, '--blackduck.url=%s' % self.baseurl, '--blackduck.api.token=' + self.token,
...
class ContainerImageScanner:
    ...
    def __init__(self, hub, container_image_name, workdir='/tmp/workdir', dockerfile=None, base_image=None,
                 omit_base_layers=False):
        self.hub = hub
    ...
    def process_container_image(self):
        ...
        for i in self.manifest[0]['Layers']:
            layer = {'name': self.image_name + "_layer_" + str(num), 'path': i}
    ...
    def generate_project_structures(self, base_layers=None):
        ...
        for layer in self.layers:
            ...
            sub_project_release = self.hub.get_or_create_project_version(layer['name'], self.image_version,
        ...
        if base_layers:
            ...
            if len(base) > 0:
                main_project_release_addon = self.hub.get_or_create_project_version(self.image_name,
                                                                                    addon_image_version)
                if not self.omit_base_layers:
                    main_project_release_base = self.hub.get_or_create_project_version(self.image_name,
                                                                                       base_image_version)
                    for layer in base:
                        parameters = {'description': layer['command']['created_by']}
                        sub_project_release = self.hub.get_or_create_project_version(layer['name'], self.image_version,
    ...
    def submit_layer_scans(self):
        for layer in self.layers:
            options = [f'--detect.project.name={layer["name"]}',
                       f'--detect.project.version.name="{self.image_version}"',
                       f'--detect.code.location.name={layer["name"]}_{self.image_version}_code_{layer["path"]}',
                       f'--detect.source.path={self.docker.imagedir}/{layer["path"].split("/")[0]}']
    ...
    def submit_single_layer_scan(self, layer_number):
        ...
        options = [f'--detect.project.name={layer["name"]}',
                   ...
                   '--detect.code.location.name={}_{}_code_{}'.format(layer['name'], self.image_version, layer['path']),
                   '--detect.source.path={}/{}'.format(self.docker.imagedir, layer['path'].split('/')[0])]
        # options.append('--detect.blackduck.signature.scanner.disabled=false')
        self.hub_detect.detect_run(options)
...
def scan_container_image_with_dockerfile(imagespec, dockerfile, base_image, omit_base_layers):
    hub = HubInstance()
    scanner = ContainerImageScanner(hub, imagespec, dockerfile=dockerfile, base_image=base_image,
...
def main(argv=None):
    ...
    parser.add_argument('--rescan-layer', default=0, type=int,
                        help="Rescan specific layer in case of failure, 0 - scan as usual")
    parser.add_argument('--dockerfile', default=None, type=str,
                        help="Specify dockerfile used to build this container(experimental), can't use with "
                             "--base-image")
    parser.add_argument('--base-image', default=None, type=str,
                        help="Specify base image used to build this container(experimental), can't use with "
                             "--dockerfile")
    parser.add_argument('--omit-base-layers', default=False, type=bool,
